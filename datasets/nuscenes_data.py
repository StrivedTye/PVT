...
class NuScenesDataset(base_dataset.BaseDataset):
    ...
    def filter_instance(self, split, category_name=None, min_points=-1):
        ...
        """
        if split != 'val' and split != 'train_track':
            split = 'val'
        if category_name is not None:
            if "nocar" in category_name.lower():
                general_classes = tracking_to_general_class["nocar"]
            elif "noped" in category_name.lower():
                general_classes = tracking_to_general_class["noped"]
            else:
    ...
    def _build_tracklet_anno(self):
        ...
        for instance in self.track_instances:
            ...
            while curr_anno_token != '':
                ...
                # validation, when training. Tye
                # if ann_record['num_lidar_pts'] < 1 and self.split == 'val':
                #     break

                # when training together , reduce frame number. Tye
                if (ann_record['num_lidar_pts'] < 1 or len(track_anno) > 50) \
                        and self.split == 'train_track' and 'all' in self.category_name:
                    break

                category_name = general_to_tracking_class[ann_record['category_name']]
                track_anno.append({"sample_data_lidar": sample_data_lidar,
                                   "box_anno": ann_record,
                                   "type": category_name})
    ...
    def _load_data2(self):
        cur_path = os.path.join(self.path, self.split, self.category_name)
        path_bank = os.path.join(cur_path, f"path_bank.dat")
        os.makedirs(cur_path, exist_ok=True)

        if os.path.isfile(path_bank):
            print(f'loading from saved file {path_bank}.')
            with open(path_bank, 'rb') as f:
                training_samples = pickle.load(f)
        else:
            print('reading from annos')
            training_samples = []
            for i in range(len(self.tracklet_anno_list)):
                ...
                for anno in self.tracklet_anno_list[i]:
                    ...
                    temp = self._get_frame_from_anno_data(anno)
                    cur_frame = os.path.join(cur_path, f"{anno['box_anno']['token']}.bat")
                    frames.append(cur_frame)

                    with open(cur_frame, 'wb') as f:
                        pickle.dump(temp, f)

                training_samples.append(frames)

            with open(path_bank, 'wb') as f:
                print(f'saving loaded data to {path_bank}')
                pickle.dump(training_samples, f)
        return training_samples
    ...
    def get_frames(self, seq_id, frame_ids):
        ...
        #     cur_path = self.training_samples[seq_id][f_id]
