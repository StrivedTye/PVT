...
class WaymoDataset(base_dataset.BaseDataset):
    '''
    for LiDAR_SOT benchmark
    '''

    def __init__(self, path, split='valid', category_name='vehicle', **kwargs):
        super().__init__(path, split, category_name)
        self.waymo_valid_folder = path
        self.benchmark_list_dir = os.path.join(self.waymo_valid_folder, 'benchmark', category_name)
        self.dataset_dir = os.path.join(self.waymo_valid_folder, 'pc', 'raw_pc')
        self.gt_info_dir = os.path.join(self.waymo_valid_folder, 'gt_info')
        self.category = category_name.lower()
        self.tracklet_anno_list, self.tracklet_len_list = self._build_tracklet_anno()
    ...
    def _build_tracklet_anno(self):
        ...
        bench_list_json = os.path.join(self.benchmark_list_dir, 'bench_list.json')
        with open(bench_list_json, 'r') as f:
            list_of_tracklet_anno = json.load(f)

        list_of_tracklet_len = []
        for tracklet_info in list_of_tracklet_anno:
            start_frame = tracklet_info['frame_range'][0]
            end_frame = tracklet_info['frame_range'][1]
            list_of_tracklet_len.append(end_frame-start_frame+1)

        return list_of_tracklet_anno, list_of_tracklet_len

    def get_frames(self, seq_id, frame_ids):
        seq_annos = self.tracklet_anno_list[seq_id]
        segment_name = seq_annos['segment_name']
        start_frame = seq_annos['frame_range'][0]
        tracklet_id = seq_annos['id']
        anno = np.load(os.path.join(self.gt_info_dir, '{:}.npz'.format(segment_name)), allow_pickle=True)  # here not support multi-process, so workers should be set to 0.
        pc_all = np.load(os.path.join(self.dataset_dir, '{:}.npz'.format(segment_name)), allow_pickle=True)

        frames = [self._get_frame_from_anno(anno, f_id+start_frame, tracklet_id, pc_all) for f_id in frame_ids]
        return frames

    def _get_frame_from_anno(self, anno, f_id, tracklet_id, pc_all):
        frame_bboxes = anno['bboxes'][f_id]
        frame_ids = anno['ids'][f_id]
        index = frame_ids.index(tracklet_id)
        ...
        pc = PointCloud(pc_all[str(f_id)])

        return {"pc": pc, "3d_bbox": bb, 'meta': {'type': self.category_name}}
