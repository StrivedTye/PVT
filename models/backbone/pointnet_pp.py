...
class Pointnet_Backbone(nn.Module):
    ...
    def __init__(self, use_fps=False, normalize_xyz=False, return_intermediate=False):
        ...
        skip_channel_list = [0, 128, 256, 256]
        self.FP_module = nn.ModuleList()
        self.FP_module.append(
            PointnetFPModule(
                mlp=[256 + skip_channel_list[-2], 128, 128],
                bn=True
            )
        )
        self.FP_module.append(
            PointnetFPModule(
                mlp=[128 + skip_channel_list[-3], 256, 256],
                bn=True
            )
        )
        self.FP_module.append(
            PointnetFPModule(
                mlp=[256 + skip_channel_list[-4], 256, 256],
                bn=True
            )
        )
    ...
    def forward(self, pointcloud, numpoints):
        ...
        # now "l_features" is [fp1, fp2, fp3, sa3]; "l_xyz" is [xyz, sa1, sa2, sa3]
        if self.return_intermediate:
            return l_xyz, l_features, l_idxs[0]
        return l_xyz[0], l_features[0], l_idxs[0]  # [B, N, 3], [B, C, N], [B, N]
