...
class Pointnet_Backbone(nn.Module):
    ...
    def __init__(self, input_channels=3, use_xyz=True):
        ...
        self.FP_modules.append(
            PointNetFeaturePropagationSA(mlp=[160, 128, 64], mlp_inte=[128, 32, 128, 64, 64]))  # 160=128+32
        self.FP_modules.append(
            PointNetFeaturePropagationSA(mlp=[192, 128, 128], mlp_inte=[128, 64, 128, 64, 128]))  # 192=128+64
...
class PointNetSetAbstractionEdgeSA(nn.Module):
    ...
    def forward(self, xyz, points, numpoints):
        ...
        if self.group_all:
            ...
            new_xyz, new_points = sample_and_group_edge(self.npoint, self.radius, self.nsample, xyz, points,
                                                        self.sampling, numpoints, False, self.use_knn)
