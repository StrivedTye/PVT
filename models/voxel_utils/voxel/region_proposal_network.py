...
class RPN(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.deconv1 = nn.Sequential(OrderedDict(
            [('ConvTranspose', nn.ConvTranspose2d(128, 128, 2, 2, 0)), ('bn', nn.BatchNorm2d(128)),
             ('relu', nn.ReLU(inplace=True))]))
        self.deconv = nn.Sequential(OrderedDict(
            [('ConvTranspose', nn.Conv2d(128, 128, 1, 1, 0)), ('bn', nn.BatchNorm2d(128)),
             ('relu', nn.ReLU(inplace=True))]))
        self.conv_final = nn.Sequential(OrderedDict(
            [('Conv', nn.Conv2d(128 * 2, 128, 1, 1)), ('bn', nn.BatchNorm2d(128)), ('relu', nn.ReLU(inplace=True))]))
