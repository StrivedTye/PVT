...
from models.voxel_utils.functional.voxelization import avg_voxelize, favg_voxelize

__all__ = ['Voxelization']


class Voxelization(nn.Module):
    def __init__(self, x, y, z, scene_ground=torch.tensor([-5.6, -3.6, -2.4]), voxel_size=torch.tensor([0.3, 0.3, 0.2]),
                 mode=True):
        super().__init__()
        self.x = int(x)
        self.y = int(y)
    ...
    def forward(self, features, coords):
        ...
        # voxel_indices = torch.cat([torch.clip(voxel_indices[:, 0:1, :], 0, self.x-1),
        #                            torch.clip(voxel_indices[:, 1:2, :], 0, self.y-1),
        #                            torch.clip(voxel_indices[:, 2:3, :], 0, self.z-1)], dim=1)
        if self.mode:
    ...
    def extra_repr(self):
        print('information:x {} y {} z {} min_voxel_coord {} voxel_size {} '.format(self.x, self.y, self.z,
