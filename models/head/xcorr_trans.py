...
from models.backbone.pointnet2_trans import LinearAttention


class STNetXCorr(nn.Module):
    def __init__(self, feat_emb, iters, attention_type, knn_num):
        super(STNetXCorr, self).__init__()

        self.cross_stage1 = corss_attention(feat_emb, iters, attention_type)
        self.local_stage1 = local_self_attention(feat_emb, iters, attention_type, knn_num)

        self.cross_stage2 = corss_attention(feat_emb, iters, attention_type)
        self.local_stage2 = local_self_attention(feat_emb, iters, attention_type, knn_num)

    def forward(self, search_feat, search_xyz, template_feat, template_xyz):
...
class local_self_attention(nn.Module):
    ...
    def forward(self, search_feat, search_xyz, mask=None):
        ...
        xyz_knn = get_graph_xyz(search_xyz.permute(0, 2, 1).contiguous(), k=self.knum, idx=kidx)  # [B, 3, N, K]
